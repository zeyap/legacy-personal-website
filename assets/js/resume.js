...
var json_cn = {
    "titles":["教育背景","技能/证书","工作&学术经历","项目经历","擅长技术","相关课程","语言能力","论文发表"],
    "contact": {
        "email": "zeyapeng@outlook.com",
    ...
    "experiences": [[
    ...
    ], [
        {
            ...
            "bulletpoints": [
                "作为数字视音频处理课程项目，在Qt Creator上用C++独立开发了听歌识曲应用的原型，借助FFTW3和QMultimedia库实现了音频分析，独立实现了乐纹提取、数据库存储、片段搜索和排序的功能，达到了较好的识曲效果",
            ]
        },
        {
            ...
            "duration": "2018年2月",
            "bulletpoints": [
                "和组员共同设计并用Unity3D制作了约会模拟游戏 <a href='https://globalgamejam.org/2018/games/dating-starts'>Dating Starts</a>，负责设计和实现交互操作与视觉反馈。最终作品被承办方（CiGa）公众号收录",
            ]
        },
        {
            "title": "空间想象能力训练游戏开发",
            ...
            "bulletpoints": [
                "设计并用Unity3D开发了训练大学新生空间想象力的3D游戏，游戏平台具有较好的可拓展性，并且每个游戏可以记录玩家行为",
        ...
        },
        {
            ...
            "bulletpoints": [
                "作为游戏程序设计课程项目，在Unity3D上共同设计并开发了AR游戏《逆向世界》，负责完成了人物造型设定，制作了游戏中所有的2D素材，完成了和交互相关的代码，学习并使用Unity Shaderlab进行游戏过场动画的特效处理",
            ]
        },
        {
            "title": "分类学习的眼动研究",
...
function formatResume(jsonObj) {
    ...
    for (var i = 0; i < exps.length; i++) {
        ...
        if (i == 0) { endOfExp1 = colNo; }
            
    }

    elems["html"][endOfExp1] += '<h4 class="timeline-title">' + titles[7] + '</h4>';
    for (var i = 0; i < publications.length; i++) {
        var pubitem = "<p>" + publications[i].title + " " + publications[i].journal + " " + publications[i].issue + "</p>";
        elems["html"][endOfExp1] += pubitem;
    }
    for (var i = 1; i <= 3; i++) {
        ...
        series: [
            {
                ...
                data: [
                    { value: 30, name: 'Interaction Design' },
                    { value: 10, name: 'Data Analysis' },
                    { value: 30, name: 'Multimedia Processing' },
                    { value: 10, name: 'Database' },
